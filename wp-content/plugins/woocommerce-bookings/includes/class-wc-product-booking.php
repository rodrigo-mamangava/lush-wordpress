...
*@return bool available or not
	...
	public function get_unavailable_mintues( $booked ) {
		...
		foreach ( $booked as $booked_block ) {
			// included the final block in the loop of the minute
			// duration type.
	...
	public function apply_buffer( $not_available_minutes ) {
		...
		foreach ( $not_available_minutes as $timestamp => $bookings_count ) {
			...
			// then we should add a buffer from this point forward for the duration of the buffer period in
			// minutes.


			$next_minute = $timestamp + $one_minute;

			if ( ! isset( $not_available_minutes[ $next_minute ] ) ) {
			...
			if ( 'yes' === $this->wc_booking_apply_adjacent_buffer
			     && ! isset( $not_available_minutes[ $previous_minute ] ) ) {
