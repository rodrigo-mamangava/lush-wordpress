...
class WC_Bookings_Controller {
	...
	public static function find_booked_day_blocks( $product_id ) {
		...
		foreach ( $existing_bookings as $existing_booking ) {
			...
			while ( $check_date < $end_date ) {
				...
				$before_midnight_tomorrow = strtotime( 'tomorrow midnight -1 min', $check_date );
				$blocks_in_range  = $product->get_blocks_in_range( $midnight, $before_midnight_tomorrow, array(), $resource_id );
				$available_blocks = $product->get_available_blocks( $blocks_in_range, array(), $resource_id );


				// Check fo fully booked
	...
	public static function get_bookings_on_date( $bookings, $date ) {
		...
		foreach ( $bookings as $booking ) {
			// does the date we want to check fall on one of the days in teh bookin?
